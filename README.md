...
- **Request duplication:** deterministic detection of the client hammering its own API — the same request (method + URL with cache-buster params stripped + body) repeated in a burst. Exactly two tightly-spaced calls are flagged gently as the React StrictMode double-invoke shape; three or more is a self-DDoS whose severity scales with the count (≥3 low, ≥6 medium, ≥12 high). Retries after a retryable/failed response (`408` / `425` / `429` / `5xx` / no response) on a fixed cadence — or a clean un-randomised exponential backoff — are flagged as **retry jitter required**, and a delay-free retry loop as high severity.
- **Recorder:** records a deterministic set of user actions (`navigate`, `click`, `change`, `keydown`, `submit`) into named, stored flows with start / stop / rename / delete. Each flow carries free-text **notes** and user **labels**; the list filters by free-text search and label (AND), and **Update** re-records a flow in place when the active tab is on the same page.
